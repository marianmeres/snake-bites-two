...
export class Board {
	...
	public readonly pubsub = createPubSub();
	...
	setPieceAtRandomEmptyXY(piece: Piece) {
	...
	atomize() {
		...
		return this.mergeGrids(
			this.createGrid((x, y) => this.cell(x, y)),
			atoms
		);
	}
	...
	update() {
		...
		pieces.sort((a: Piece, b: Piece) => a.preUpdateSortOrder - b.preUpdateSortOrder);

		// now update all
		pieces.forEach((piece: Piece) => piece.update());
	...
	resetScores() {
		this.forEachXY((x, y) => {
			const cell = this.cell(x, y);
			if (cell && cell instanceof Snake) cell.score.set(0);
		});
	}
